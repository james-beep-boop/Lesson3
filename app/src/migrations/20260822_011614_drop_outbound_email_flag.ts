...
/**
 * Drop `features_outbound_email` — the flag is removed, not renamed (operator decision 2026-08-21).
 *
 * ⚑ THE FILENAME SAYS `20260822`; THE DECISION WAS THE 21st. Payload stamps a migration name from
 * UTC, and this was generated at 01:16 UTC — 18:16 the previous evening where the decision was made.
 * The name is the identifier stored in the `payload_migrations` table, so it stays as generated; the
 * prose dates are the real ones. Same for `20260821_234341_add_system_settings`, an hour and a half
 * earlier by the clock that matters.
 *
 * ⚑ SAFE TO DROP BECAUSE NOTHING EVER READ IT. It was added an hour and a half earlier by
 ...
 * what the column means — a record of a decision about something real (operator review, 2026-08-21).
