...
5. Deferred, in rough value order: catalogue/admin pagination at scale; the recipient roster's
   ...
   hosted-Sentry DSN). Also consider a **scheduled deps-audit job** — four unrelated transitive advisories
...
`SERVER_URL`/`ADMIN_URL` → the public URL (still the Tailscale URL), edge rate limiting, hosted Sentry.
