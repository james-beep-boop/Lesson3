...
session. PR **#333** then passed the required gate and merged the deployment follow-up as `f67c75f`.
No deployment was performed in either session. Work is in `/Users/jamesmcclelland/Developer/Lesson3`;
do not use the older Documents/GitHub checkout by assumption. Measure repository and deployment state
again before taking the next action.
...
The deployment follow-up separately passed `bash -n`, `git diff --check`, all 13 deploy branch cases,
...
Deploy current `origin/main` with the normal `scripts/deploy.sh`. No migration file or persisted field
changed; the one-shot migrate service still runs and should report no pending migration. Read the
script's sidecar message rather than predicting whether Gotenberg will rebuild: it measures the
checked-out tree against the installed image on that box. The script waits for `/login` before
reporting `OK`; afterward, verify one successful Save as a Teacher
...
⚑ **THIS BLOCK NAMES NO `main` SHA, DELIBERATELY.** Every previous handoff hard-coded one, and every
one was stale within hours — this one twice in an evening, because a commit cannot contain the squash
SHA it will produce. **Measure instead:** `git fetch -q origin main && git rev-parse --short origin/main`.
If the newest merged PR is #321 or later, this block is current.
...
| Deployed | ⚑ **UNKNOWN — measure, do not trust any line here.** The operator deployed at least twice during the session; how far the Rock got is not recorded because every figure written here went stale the same evening. `git fetch -q origin main && git log --oneline $(ssh Rock5b 'git -C /srv/lesson3 rev-parse HEAD')..origin/main` — against `origin/main` after a fetch, not the local ref |
...
Started as an audit of #302–#308 and became a day of editor and phone fixes. #309–#317 are the
product changes; #318, #320 and #321 are docs/test work, and #319 hid Payload's clipboard menu.
...
⚑ **BACK BURNER, GATED ON A SPEC AMENDMENT: "WHO MANAGES WHAT."** A view of who administers each
subject-grade and who holds editing access there, with a message button per person. Operator request
2026-08-29, discussed and deliberately not built. ⚑ The gating item is **a SPEC §8 amendment that has
not been made** — teachers currently *cannot* know who the administrators are, and
`endpoints/requestEditing.ts` says that is deliberate, so this is a reversal of a named decision
rather than a new screen. Do not start until the amendment exists. Full plan, including the
recommended audience split, the DAG-not-tree finding, costs, and three questions left open:
`docs/DESIGN-who-manages-what-2026-08-29.md`.

1. **Deploy whatever the Rock is missing** — measure it with the command in the Status table rather
   than trusting a count. ⚑ Nothing here needs a migration. ⚑ And note that the docs PRs are NOT all
   deploy-free: #321 changed `(frontend)/guide/page.tsx`, a live route, so the corrected in-app guide
   does not reach users until the app is deployed.
2. **The Back-from-Word bug**, with the ruled-out list as the starting point.
3. ⚑ **The array row `⋯` menu, plus Add Lesson** — they offer teachers Move / Add below / Duplicate /
   **Remove**, which SPEC §5 makes Subject-Admin-only. ⚑ **The server REJECTS these with a 403** —
   `fieldSplit.ts`'s structural guard throws `Forbidden` on any cardinality or order change. (An
   earlier version of this line said "silently discarded"; that was my untested guess and it was
   wrong.) So it is not an authorization hole — the teacher composes an apparently valid edit, presses
   Save, and is refused after doing the work. The fix is to stop OFFERING the controls, which makes
   this a UI change, not a permissions one. DECISIONS 2026-08-29 ("OPEN: the array row ⋯ menu").
...
`docs/DESIGN-manage-accordion-2026-08-16.md` — agreed with the operator, reviewed externally **six
